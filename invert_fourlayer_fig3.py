# -*- coding: utf-8 -*-
"""
Created on Wed Jan 10 15:19:41 2024
...
k = 0.1  # scale factor in Fig. 2

capr1[0] = 0.5  # R1 = 0.5 m
for i in range(capm):
    capr1[i] = capr1[0] * (1 + k) ** i

for i in range(capm):
    t[i] = 1.0 / 16 * 0.0254  # 1/16th of an inch
    mur[i] = 20000
...
for i in range(rank):
    for j in range(rank):
        if j < i:
            element = (n / (n + 1)) * (r[j] / r[i]) ** (2 * n + 1)
        elif j > i:
            element = -1
        else:  # j == i
            if (i + 1) % 2 == 1:
                m = ((i + 1) + 1) // 2 - 1
                element = -((n + 1) * mur[m] + n) / ((n + 1) * (mur[m] - 1))
            else:
                m = (i + 1) // 2 - 1
                element = (n * mur[m] + (n + 1)) / ((n + 1) * (mur[m] - 1))
        a[i, j] = element
        print(f"{element} ", end="")
    print()
